...
"F#9" => [11839.82, 13.06, 126], "G9" => [12543.85, 13.07, 127]
	...
	class Note
		...
		def to_hz; PITCHES["#{self.note.upcase}#{self.octave.to_s}"][0]; end

		# returns the midi note for this note
		def to_MIDI; PITCHES["#{self.note.upcase}#{self.octave.to_s}"][2]; end
	end 
	...
	class ChromaticScale
		...
		def <=> (other); return self.root <=> other.root; end
		# returns each notes of the scale
		def each; @chromatic_scale.each { |e| yield e  }; end
	
		def to_s; @chromatic_scale.inject('') {|mem, n| mem << n.note.to_s<<' '<< n.octave.to_s<<', ' }; end 
	...
	class Chord < ChromaticScale
		...
		def name;"#{@root}#{@octave} #{(@type).to_s.capitalize} inversion: #{@inversion}"; end
	end 
